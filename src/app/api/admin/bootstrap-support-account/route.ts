 ...
 * um Administrador Geral aprovar essa delegação na tela normal de Delegações
 * (aba Pendentes). É essa aprovação que representa a "ativação".
 *
 * Só quem já é Administrador Geral pode chamar essa rota.
 */
...
export async function POST(req: Request) {
  ...
  // 1) Verifica autor: precisa ser Administrador Geral
  ...
  if (!callerProfile || callerProfile.role !== "adm_general") {
    return NextResponse.json({ error: "Apenas o Administrador Geral pode provisionar essa conta." }, { status: 403 });
  }
