...
import type {
  LgScoreMinisterial, LgRanking, RetentionFunnel, RetentionFunnelByChurch,
  LgReliabilityIndex, ReliabilitySummary, MonthlyConsolidation, GrowthVariation,
} from "@/types/domain";
...
export async function getRetentionFunnelByChurch(sb: SupabaseClient): Promise<RetentionFunnelByChurch[]> {
  ...
  if (error) { console.error("[retention_funnel_by_church]", error); return []; }
...
export async function getMonthlyConsolidation(
  sb: SupabaseClient,
  opts?: { churchId?: string; mes?: string }
