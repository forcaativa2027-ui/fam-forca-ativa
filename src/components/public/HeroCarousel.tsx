...
export function HeroCarousel({ banners, fallback, intervalMs = 6000, onSeeVideos, onSeeServices }: Props) {
  ...
  return (
    <section
      className="fam-carousel-surface relative overflow-hidden rounded-2xl shadow-[0_14px_40px_rgba(74,23,63,0.18)]"
      style={b.image_url ? { backgroundImage: `linear-gradient(90deg, rgba(74,23,63,0.94) 0%, rgba(74,23,63,0.72) 48%, rgba(50,19,45,0.28) 100%), url(${b.image_url})`, backgroundSize: "cover", backgroundPosition: "center" } : undefined}
    >
      <div className="relative min-h-[280px] p-8 pb-14 md:min-h-[360px] md:p-12 md:pb-16">
        <span className="inline-block rounded-full border border-gold px-3 py-1 text-[11px] font-extrabold tracking-widest text-gold">FAM · FORÇA ATIVA DA MULHER</span>
        ...
        {b.cta_label && b.cta_url && (
          <div className="mt-6">
            <Button asChild className="gap-2 bg-[#D93683] text-white shadow-lg shadow-[#D93683]/25 hover:bg-[#B52A6E]"><a href={b.cta_url} target={b.cta_url.startsWith("http") ? "_blank" : undefined} rel="noreferrer">{b.cta_label}</a></Button>
          </div>
        )}
      </div>
      ...
      {total > 1 && (
        <>
          ...
          <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-2 rounded-full bg-black/15 px-3 py-2 backdrop-blur-sm">
            {active.map((_, idx) => (
              <button key={idx} onClick={() => setI(idx)} aria-label={`Slide ${idx+1}`}
                className={`h-2 rounded-full transition-all ${idx === i ? "w-8 bg-[#E8C978]" : "w-2 bg-white/45"}`} />
            ))}
          </div>
        </>
      )}
...
function DefaultHero({ onSeeVideos, onSeeServices, fallback }: { onSeeVideos?: () => void; onSeeServices?: () => void; fallback?: React.ReactNode }) {
  ...
  return (
    <section className="fam-carousel-surface relative overflow-hidden rounded-2xl p-8 shadow-[0_14px_40px_rgba(74,23,63,0.18)] md:p-12">
      <span className="inline-block rounded-full border border-gold px-3 py-1 text-[11px] font-extrabold tracking-widest text-gold">FAM · FORÇA ATIVA DA MULHER</span>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {onSeeVideos   && <Button onClick={onSeeVideos}   className="gap-2 bg-[#D93683] text-white hover:bg-[#B52A6E]"><Play className="h-4 w-4" /> Ver pregações</Button>}
        {onSeeServices && <Button variant="outline" onClick={onSeeServices} className="border-gold/70 bg-transparent text-white hover:bg-gold/15 hover:text-white">Horários de culto</Button>}
      </div>
