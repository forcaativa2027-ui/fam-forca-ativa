...
export default function ForgotPasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!email || !/\S+@\S+\.\S+/.test(email)) {
      setErr("Informe um e-mail válido."); return;
    }
    setBusy(true); setErr("");
    ...
    <main className="relative grid min-h-screen place-items-center bg-[radial-gradient(circle_at_30%_20%,#16345A,#0E2A47_60%)] p-5">
      ...
      <div className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-2xl">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-display text-2xl text-[#0E2A47]">Recuperar senha</h1>
        </div>
        ...
        {sent ? (
          <div className="space-y-4 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Verifique sua caixa de entrada (e o spam). O link para redefinir a senha expira em <strong>1 hora</strong>.
            </p>
            <Link href="/entrar">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <p className="text-center text-xs text-muted-foreground">
              Lembrou a senha?{" "}
              <Link href="/entrar" className="font-semibold text-[#0E2A47] hover:underline">Entrar</Link>
            </p>
          </form>
        )}
      </div>
