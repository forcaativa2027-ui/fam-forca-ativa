...
export default function ResetPasswordForm() {
  ...
  // Ouvir o evento PASSWORD_RECOVERY confirma que o token é válido.
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
      if (event === "PASSWORD_RECOVERY") {
        setStage("form");
      } else if (event === "SIGNED_IN") {
        // pode chegar antes do PASSWORD_RECOVERY; aguardar
      }
    });
    // Timeout: se não chegar o evento em 5s, token inválido/expirado
    ...
    <main className="relative grid min-h-screen place-items-center bg-[radial-gradient(circle_at_30%_20%,#16345A,#0E2A47_60%)] p-5">
      ...
      <div className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-2xl">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-display text-2xl text-[#0E2A47]">Nova senha</h1>
        </div>
        <div className="my-3 h-[3px] w-16 rounded bg-[#C9A227]" />

        {stage === "loading" && (
          <p className="text-sm text-muted-foreground text-center py-6">Verificando link…</p>
        )}

        {stage === "invalid" && (
          <div className="space-y-4 text-center">
            <AlertTriangle className="mx-auto h-12 w-12 text-amber-500" />
            <p className="font-semibold text-[#0E2A47]">Link inválido ou expirado</p>
            <p className="text-sm text-muted-foreground">Solicite um novo link de recuperação.</p>
        ...
        {stage === "success" && (
          <div className="space-y-4 text-center">
            ...
            <p className="font-semibold text-[#0E2A47]">Senha redefinida!</p>
