 ...
 */
const PROFILES = [
  { key: "padrao", label: "Padrão", icon: Sparkles, desc: "A experiência recomendada pela plataforma", font: "media" as const },
  { key: "idoso", label: "Perfil Idoso", icon: User, desc: "Fonte maior, mais espaço, mais fácil de ler", font: "grande" as const },
  { key: "baixa_visao", label: "Baixa Visão", icon: Glasses, desc: "Fonte extra grande e alto contraste", font: "extra_grande" as const },
  { key: "simplificado", label: "Simplificado", icon: LayoutList, desc: "Menos elementos na tela, navegação mais direta", font: "grande" as const },
];

export function AccessibilityOnboarding() {
  const { onboarded, loaded, setFontSize, markOnboarded } = useAccessibility();

  if (!loaded || onboarded) return null;

  function choose(font: "media" | "grande" | "extra_grande") {
    setFontSize(font); markOnboarded();
  }
