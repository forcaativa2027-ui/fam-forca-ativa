...
export function BottomNav({ items, activeKey }: { items: BottomNavItem[]; activeKey: string }) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-fam-gold/45 bg-fam-plum text-white shadow-[0_-6px_20px_rgba(74,23,63,0.22)]">
      <div
        ref={scrollerRef}
          ...
          return (
            <button
              ...
              className={`flex shrink-0 snap-center flex-col items-center gap-0.5 rounded-lg px-3 py-1.5 transition ${
                active ? "bg-fam-gold/20 text-fam-gold-soft" : "text-white/75 hover:bg-white/10 hover:text-white"
              }`}
            >
