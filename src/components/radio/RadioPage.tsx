...
// Ícones por categoria - usando componentes importados do lucide-react
...
export default function RadioPage() {
  const { data: config = null } = useRadioConfig();
  const { data: programs = [], isLoading } = useRadioPrograms();
                ...
                {programs.map((p) => (
                  <div key={p.id} className="p-4 rounded-xl border border-gray-200 hover:border-gold transition-colors">
                    <div className="flex items-center gap-3">
                      <CategoryIcons[p.category as keyof typeof CategoryIcons] className="shrink-0 h-6 w-6 text-navy" />
